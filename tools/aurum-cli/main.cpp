...
constexpr const char *REF_CACHE_PATH = "/tmp/aurum-cli-refmap.tsv";
...
void printUsage()
{
    std::cout
        ...
        << "Reference target: pass @eN from latest snapshot cache (example: aurum-cli click @e1)\n";
}
...
bool saveRefCache(const std::unordered_map<std::string, std::string> &refToId)
{
    std::ofstream out(REF_CACHE_PATH, std::ios::trunc);
    if (!out.is_open())
        return false;

    for (const auto &entry : refToId) {
        out << entry.first << "\t" << entry.second << "\n";
    }
    return true;
}

std::unordered_map<std::string, std::string> loadRefCache()
{
    std::unordered_map<std::string, std::string> refToId;
    std::ifstream in(REF_CACHE_PATH);
    if (!in.is_open())
        return refToId;
    ...
    while (std::getline(in, line)) {
        ...
        if (tabPos == std::string::npos)
            continue;
        auto ref = line.substr(0, tabPos);
        auto id = line.substr(tabPos + 1);
        if (!ref.empty() && !id.empty()) {
            refToId[ref] = id;
        }
    }
    return refToId;
                                                    ...
                                                    const std::string &rawRef)
{
    ...
    const auto refToId = loadRefCache();
    const auto iter = refToId.find(refKey);
    if (iter == refToId.end())
        return nullptr;

    auto selector = std::make_shared<Aurum::UiSelector>();
    selector->id(iter->second);
                                                  ...
                                                  std::string &detail)
{
    ...
    if (refIter != opts.end()) {
        ...
        if (!refObj) {
            ...
            detail = "ref not found (run snapshot first to refresh cache)";
            return nullptr;
        }
        return refObj;
    }
                         ...
                         std::unordered_map<std::string, std::string> &refToId)
{
    ...
    if (shouldIncludeInSnapshot(node, role)) {
        ...
        auto uiObj = std::make_shared<Aurum::UiObject>(device, nullptr, node);
        const auto id = uiObj->getId();
        if (!id.empty()) {
            refToId[refKey] = id;
        }
    }
    ...
    for (const auto &child : node->getChildren()) {
        appendSnapshotLines(device, child, refIndex, out, refToId);
    }
}
...
int runSnapshot(const std::shared_ptr<Aurum::UiDevice> &device, const Options &opts)
{
    ...
    for (const auto &root : roots) {
        appendSnapshotLines(device, root, refIndex, out, refToId);
    }

    if (!saveRefCache(refToId)) {
        std::cerr << "status=error code=ACTION_FAILED detail='failed to write ref cache'\n";
        return EXIT_ACTION_FAILED;
    }
