...
- `aurum-cli click @e1` (use ref from `snapshot`; ref cache resolves by `id`)
...
- For post-snapshot interactions, support reference targets like `@e1` via file cache (`ref -> id`).
- On every `snapshot`, clear and rewrite the ref cache.
