...
- ref targets: snapshot writes `ref -> id` cache file, and commands like `click @e1` resolve by loading cache then finding with `UiSelector::id(...)`.
- lifecycle: each new `snapshot` clears and rewrites the cache.
