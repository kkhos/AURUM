...
def PickerExecuteTestWithText(stub):
    ...
    if len(response.elements) <= 0: return
    #print(response)
    rect = stub.getSize(ReqGetSize(type='WINDOW', elementId=response.elements[0].elementId))
    print(rect)
    # 2
    ...
    stub.click(ReqClick(type='ELEMENTID', elementId=targetObj))
    # 5
    response = stub.findElement(ReqFindElement(textField='Black', isShowing=True))
    if len(response.elements) <= 0: return False
