...
std::vector<std::shared_ptr<AccessibleNode>> UiDevice::getWindowRoot() const
{
    ...
    std::unordered_set<int> addedPids{};
    std::map<int, std::shared_ptr<AccessibleApplication>> pidToAppNode{};
    ...
    for (auto tWin : mTizenWindows)
    {
        ...
        auto app = pidToAppNode[tWin->getPid()];
        if (app != nullptr)
        {
            if (addedPids.count(app->getAccessibleNode()->getPid()) == 0)
            {
                addedPids.insert(app->getAccessibleNode()->getPid());
