...
gboolean AtspiWrapper::Atspi_accessible_set_listen_render_post(AtspiAccessible *obj, gboolean enabled, GError **error)
{
    std::unique_lock<std::recursive_mutex> lock(mMutex);
    return atspi_accessible_set_listen_render_post(obj, enabled, error);
}