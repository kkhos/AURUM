...
{
    LOGI("Request window info %d %d", !AccessibleWatcher::getInstance()->getWindowChanged(), mCachedNode.size());

    if (!AccessibleWatcher::getInstance()->getWindowChanged() && mCachedNode.size() > 0)
    {
    ...
    mCachedNode = ret;
    AccessibleWatcher::getInstance()->setWindowChanged();
    return ret;
}
